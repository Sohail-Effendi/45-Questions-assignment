...
const things: { type: string, model: string, year: number } = {
    type: "Honda",
    model: "Honda City",
    year: 2006
  };
  ...
  const names: (string | number) [ ] = ["Ali", "Saad", "Zain", 1, 2, 5];
  console.log(names[6]);
  ...
  // 29. Favorite Fruit: 
  const favoritFruits: string[]= ["Apricote", "Banana", "Orange"];
  ...
  //30. Hello Admin: 
      let users1:string[] = ['Ali', 'Saad', 'Zain', 'Bisma'];
